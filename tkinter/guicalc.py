...
class App(tk.Tk):
    def __init__(self):
        ...
        self.combobox = ttk.Combobox(self, values=["+", "-", "*", "/"], state="readonly")
        ...
        self.calculateButton = tk.Button(self, text="Vypočítat", command=self.showCalculated)
    ...
    def calculate(self, lambdaFunc):
        try:
            self.calculated.config(text=lambdaFunc(float(self.num1.get()), float(self.num2.get())))
        except ValueError:
            self.calculated.config(text="Chyba!")
    
    def showCalculated(self, event=None):
